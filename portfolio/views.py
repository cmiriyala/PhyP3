...
def portfolio(request,pk):
    ...
    #for mutual fund
     sum_purchased_value_mutual = Mutualfund.objects.filter(customer=pk).aggregate(Sum('purchased_value'))
     sum_recent_value_mutual = Mutualfund.objects.filter(customer=pk).aggregate(Sum('recent_value'))


     return render(request, 'portfolio/portfolio.html', { 'customer': customer, 'investments' :investments,'stocks' : stocks,  'mutualfunds' :mutualfunds,
                                                          'sum_acquired_value': sum_acquired_value, 'sum_recent_value': sum_recent_value,
                                                          'sum_purchased_value_mutual':sum_purchased_value_mutual, 'sum_recent_value_mutual': sum_recent_value_mutual
                                                           })
